...
func newKgIndexCmd(_ *config.Config, log *zap.Logger) *cobra.Command {
	...
	cmd.Flags().StringVar(&langFlag, "lang", "", "language filter: go,java,javascript,html (default: all)")
	cmd.Flags().IntVar(&jobs, "jobs", 0, "number of parallel workers (default: NumCPU)")
	return cmd
}

// parseLangFlag converts a comma-separated lang string (e.g. "go,java,javascript,html") to []pkgkg.Lang.
...
func parseLangFlag(flag string) []pkgkg.Lang {
	...
	for _, p := range parts {
		...
		case "html":
			langs = append(langs, pkgkg.LangHTML)
		}
	}
	return langs
}
