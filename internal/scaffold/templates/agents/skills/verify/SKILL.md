...
- Forbidden words in completion claims: "should", "probably", "seems to", "likely", "I believe", "I think it works".
  These signal unverified assertions.
...
After a failed verification, store the failure pattern:
`aikits memory store --title "<failure pattern>" --content "<what failed and how to avoid>" --tags "verify,failure-pattern"`
