...
3. **Clarify and explore (loop until converged)**:
    - **Ask clarification questions** for every gap, contradiction, or ambiguity. Do not just list issues — actively ask
      specific questions to resolve them. Example: "Success criteria say 'fast' — what's the target latency?"
    - **Brainstorm and explore options** — For key decisions, trade-offs, or areas with multiple viable approaches,
      proactively brainstorm alternatives. Present options with pros/cons and trade-offs. Don't just accept the first
      approach — challenge assumptions and surface creative alternatives.
    ...
    - **Backward trigger**: if 3 or more of the following are true — missing problem statement, undefined success
