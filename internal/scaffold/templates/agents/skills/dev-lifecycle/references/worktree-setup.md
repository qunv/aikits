...
8. Bootstrap dependencies before any phase work (be proactive and project-specific):
   - **Skip if already bootstrapped**: check for evidence of a prior install (e.g., `node_modules/`, `.venv/`,
     `vendor/`) and compare the manifest/lockfile modification time against these directories. If the install directory
     is newer than the manifest, skip bootstrap and note it was skipped.
