---
name: memory
description: aikits · Use the memory CLI as a durable knowledge layer. Search before non-trivial work, store verified reusable knowledge, update stale entries, and avoid saving transcripts, secrets, or one-off task progress.
---

# aikits Memory CLI

Use `aikits memory ...` as the durable knowledge layer.

## Workflow

1. For implementation, debugging, review, planning, or documentation tasks, search before deep work unless the task is trivial:
   ```bash
   aikits memory search --query "<task, subsystem, error, or convention>" --limit 5
   ```
...
3. Search before storing:
   ```bash
   aikits memory search --query "<knowledge to store>"
   ```
...
Use `--tags` to boost matching results (comma-separated). The `id` field is shown in results and can be used for updates:

```bash
aikits memory search --query "<query>"
```

Options: `--query/-q` required; `--tags` (boost, comma-separated); `--scope/-s`; `--limit/-l` from 1-20.
...
Find the ID from `search` results, then update only changed fields:

```bash
aikits memory update \
  --id "<memory-id>" \
  --title "<updated title>" \
...
- CLI missing: run `aikits --version`.
...
- DB path: default is `~/.aikits/memory.db`; use `--db` to override.