...
// InsertDeclaresEdges inserts DECLARES edges for Java symbols in the given repo.
// A package DECLARES its direct top-level named symbols (classes, interfaces,
// enums) — anything that is not a package, method, field, or constructor.
...
func InsertDeclaresEdges(sqlDB *sql.DB, repoID int64) error {
	...
	if _, err := sqlDB.Exec(q, repoID); err != nil {
		return fmt.Errorf("java InsertDeclaresEdges: %w", err)
	}
	return nil
}
