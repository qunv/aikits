...
// InsertImportsEdges rebuilds IMPORTS edges for Go packages in the given repo.
// Deletes existing extractor-generated IMPORTS edges whose source is a Go symbol,
...
func InsertImportsEdges(sqlDB *sql.DB, repoID int64) error {
	...
	`, repoID, repoID); err != nil {
		return fmt.Errorf("go delete old import edges: %w", err)
	}
	...
	if _, err := sqlDB.Exec(`
		...
		WHERE r.repo_id = ?
		  ...
		  AND pkg.lang = 'go'
	`, repoID); err != nil {
		return fmt.Errorf("go insert import edges: %w", err)
	}
	return nil
}
