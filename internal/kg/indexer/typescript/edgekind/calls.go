...
// InsertCallsEdges inserts heuristic CALLS edges for TypeScript callsites in the given file.
// Matches callee_text to TypeScript symbols by last component (handles "obj.method", "fn").
...
// InsertBulkCallsEdges rebuilds all heuristic CALLS edges for TypeScript across the entire repo.
...
func InsertBulkCallsEdges(db *sql.DB, repoID int64) error {
	_, err := db.Exec(`
		...
		SELECT
			...
			datetime('now')
		FROM callsites cs
		JOIN symbols caller ON caller.id = cs.caller_symbol_id AND caller.lang = 'typescript'
		JOIN symbols s ON s.repo_id = cs.repo_id
		WHERE cs.repo_id = ?
		  AND cs.caller_symbol_id IS NOT NULL
		  AND s.lang = 'typescript'
	...
	`, repoID)
	if err != nil {
		return fmt.Errorf("ts InsertBulkCallsEdges: %w", err)
	}
	return nil
}
